...
acs_est <- get_acs(geography = "tract", state = state_fips[1], variables = acsvars, year = 2017, survey = "acs5", cache_table = TRUE, output = "wide", geometry = TRUE,
                   keep_geo_vars = TRUE)
...
# With geography
st_write(acs_estimates, "./data/working/acs_2013-17/calc/acs_2013-17_calc_tract.shp", driver = "ESRI Shapefile")  # create to a shapefile 

# Data only
st_geometry(acs_estimates) <- NULL

write.csv(acs_estimates, file = "./data/working/acs_2013-17/nogeo/acs_2013-17_calc_tract.csv", row.names = F)
